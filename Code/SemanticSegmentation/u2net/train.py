    ...
    for epoch in range(args.start_epoch, args.epochs):
        ...
        if epoch % args.eval_interval == 0 or epoch == args.epochs - 1:
            ...
            if current_mae >= mae_info and current_f1 <= f1_info:
                torch.save(save_file, Weight_save_path + "/model_best.pth")

        # only save latest 10 epoch weights
        if os.path.exists(f"{Weight_save_path}/model_{epoch-10}.pth"):
            os.remove(f"{Weight_save_path}/model_{epoch-10}.pth")

        torch.save(save_file, f"{Weight_save_path}/model_{epoch}.pth")
...
def parse_args():
    ...
    parser.add_argument("--data-path", default=DataSet_path, help="DUTS root")
